...
export class MentorDetailsRepositoryImpl implements IMentorProfileRepository {
   ...
   async findAllMentors({ page = 1, limit = 10, search = "", status = "" }) {
      try {
         ...
         const match: FilterQuery<IMentorProfileModel> = {};
         if (status) match["userId.mentorRequestStatus"] = status;
         if (search) {
            match.$or = [{ "userId.firstName": { $regex: search, $options: "i" } }, { "userId.lastName": { $regex: search, $options: "i" } }, { professionalTitle: { $regex: search, $options: "i" } }, { "userId.skills": { $regex: search, $options: "i" } }];
         }
         ...
         const mentors = mentorsRaw.map(
            (doc: AggregatedMentorDoc) =>
               new MentorEntity({
                  ...
                  badges: doc.user.badges ? doc.user.badges.map((id: any) => id.toString()) : null,
