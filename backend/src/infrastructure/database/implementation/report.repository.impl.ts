...
export class ReportRepositoryImpl implements IReportRepository {
   ...
   async updateStatus(reportId: string, status: string): Promise<ReportEntity> {
      try {
         const updated = await ReportModel.findByIdAndUpdate(reportId, { status }, { new: true });
         if (!updated) throw new Error("Report not found");
         return ReportEntity.fromDbDocument(updated as any);
      } catch (error) {
   ...
   async findAll(page: number, limit: number, searchTerm?: string, status?: string): Promise<{ tasks: ReportEntity[]; totalCount: number }> {
      try {
         ...
         const docs = await ReportModel.find(query).sort({ createdAt: -1 }).skip(skip).limit(limit);

         return { tasks: docs.map((doc) => ReportEntity.fromDbDocument(doc as any)), totalCount };
      } catch (error) {
