...
export class WalletRepositoryImpl implements IWalletRepository {
   ...
   async createTransaction(data: { fromUserId: string | null; toUserId: string; fromRole: RoleEnum; toRole: RoleEnum; amount: number; type: TransactionsTypeEnum; purpose: string; method: TransactionMethodEnum; description?: string; sessionId?: string | null }): Promise<WalletTransactionEntity> {
      try {
   ...
   async getTransactionsByUser(userId: string, page: number = 1, limit: number = 10, filter: Record<string, string | number | Date> = {}): Promise<{ data: WalletTransactionEntity[]; total: number }> {
      try {
   ...
   async revenueChartData(
      ...
      try {
         /* ---------- 1. Match filter ------------------------------------ */
         const matchFilter: FilterQuery<IWalletTransactionDocument> = {
            toUserId: new mongoose.Types.ObjectId(adminId),
            toRole: "admin",
            type: "credit",
            purpose: { $in: [TransactionPurposeEnum.PLATFORM_FEE, TransactionPurposeEnum.PLATFORM_COMMISSION] },
