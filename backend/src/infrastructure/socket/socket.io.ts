...
const initializeSocket = (io: Server, SessionModel: Model<ISessionDocument>) => {
	io.on("connection", (socket: Socket) => {
		...
		socket.on("reconnect-session", async ({ sessionId, userId, peerId, role, name, avatar }: { sessionId: string; userId: string; peerId: string; role: "mentor" | "user"; name: string; avatar?: string }) => {
			try {
				const session = await SessionModel.findById(sessionId);
				if (!session) {
