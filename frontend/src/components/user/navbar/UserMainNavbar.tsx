...
export function UserMainNavbar({ isAuthenticated }: { isAuthenticated: boolean }) {
				...
				<nav className="hidden md:flex items-center gap-6">
					...
					<Link
						...
						className={cn(
							...
							pathname === "/browse" || pathname.startsWith("/browse/") ? "text-primary scale-105 font-bold" : "text-muted-foreground hover:text-primary hover:scale-105"
						)}>
						Browse Mentors
					</Link>
					<Link
