...
export const VideoCallPage = () => {
	...
	const [joinRequests, setJoinRequests] = useState<{ userId: string; sessionId: string; peerId: string }[]>([]);
	...
	useEffect(() => {
		...
		socket.on("join-request", ({ userId, sessionId: requestSessionId, peerId }: { userId: string; sessionId: string; peerId: string }) => {
			console.log("Received join-request:", { userId, requestSessionId, peerId, role }); // Debug log
			if (role === "mentor" && requestSessionId === sessionId) {
				setJoinRequests((prev) => {
					const newRequest = { userId, sessionId: requestSessionId, peerId };
				...
				toast.info(`User ${userId} has requested to join the session`);
			} else {
			...
			<div className="flex-1 relative">
				...
				{role === "mentor" && (
					<JoinRequestDrawer
						...
						onClose={() => setShowJoinRequestDrawer(joinRequests.length > 0 ? true : false)}
...
interface JoinRequestDrawerProps {
	isOpen: boolean;
	onClose: () => void;
	joinRequests: { userId: string; sessionId: string; peerId: string }[];
	...
	return (
		<Drawer
			...
			<DrawerContent className="w-[400px] h-full fixed right-0 top-0">
						...
						joinRequests.map((request) => (
							<div key={request.userId} className="flex items-center justify-between p-2 border-b">
								<div>
									...
									<p className="text-sm text-muted-foreground">Requested to join session</p>
								</div>
				...
				<DrawerFooter>
					<DrawerClose asChild>
						<Button variant="outline" disabled={joinRequests.length > 0}>
							Close
						</Button>
					</DrawerClose>
				</DrawerFooter>
